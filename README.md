...
- 指導記録の授業内容・所感は「詳細を見る」ボタンでアニメーション付きに開閉（普段はタイトルだけの表示）
...
- **チューター向け「今日やること」ダッシュボード**：タブの上に、担当する全生徒を横断して「今日の指導予定」
  「期限が近い（3日以内）未提出の宿題」「支払日が近い未確認の月謝」をまとめて表示。項目をタップすると
  その生徒に切り替わってカレンダータブが開く
...
※ `firestore.rules` を更新した（例：「今日やること」ダッシュボードの追加）ときは、コンソールの
「ルール」タブに最新の内容を貼り直して**公開ボタンを押す**必要があります。反映し忘れると、
ダッシュボードに「読み込みに失敗しました」と表示されます。
