...
- **入口**：「学生・保護者です」／「チューターです」の2択 → パスコード入力
  （一度入力すればその端末では以後スキップ）
- **宿題**：チューターが出題、生徒が提出チェック。達成度を円形グラフで表示。カレンダーで期限を一覧
- **指導記録**：日付／教科／学習範囲／内容／所感（チューターが入力、全員が閲覧）。指導予定（日時・Zoom URL）の登録とカレンダー表示
- **勉強時間**：登録した参考書ごとに学習時間を記録・集計
- **参考書**：名前・画像・教科を登録し、進捗（%）を円形グラフで表示
- **成績**：模試ごとに教科別の点数（素点／満点）を登録し、教科別の得点率推移をグラフ表示
- **連絡**：チューターと学生・保護者の共有メッセージ欄
- **設定**：プロフィール（名前・誕生日・志望校・得意/苦手科目）／チューターのみ：生徒の追加・生徒別パスコード設定・自分のパスコード変更
